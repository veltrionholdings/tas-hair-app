...
function AdminBookingsPage() {
  ...
  async function loadBookings() {
    try {
      ...
      if (filter === 'today') {
        ...
        params = { from: tomorrow, to: dayAfterTomorrow };
      } else if (filter) {
        params = { status: filter };
      }

      const result = await api.getBookings(params.status ? { status: params.status } : { from: params.from, to: params.to });
  ...
  function formatDate(isoString: string): string {
    try {
      return new Date(isoString).toLocaleDateString('en-ZA', { weekday: 'short', day: 'numeric', month: 'short' });
  ...
  function getServiceName(booking: Booking): string {
    if (typeof booking.service === 'object' && booking.service) {
            ...
            {bookings.map(booking => (
              <div key={booking.id} className="admin-booking-card card">
                ...
                <div className="admin-booking-customer">
                  {booking.resource && typeof booking.resource === 'object' && <span>💇‍♀️ {booking.resource.name}</span>}
